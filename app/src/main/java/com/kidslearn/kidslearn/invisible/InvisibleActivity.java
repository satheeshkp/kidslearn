package com.kidslearn.kidslearn.invisible;
 ...
 */
public class InvisibleActivity extends AppCompatActivity implements CvCameraViewListener2 {
    /**
...
//        activityManager = (ActivityManager) getSystemService(ACTIVITY_SERVICE);
    ...
    public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
        ...
        if(counter<60){
            background = inputFrame.rgba();
//            Core.flip(background.t(), background, 1); //mRgba.t() is the transpose
            counter++;
        ...
        if(img==null){
            ...
            Imgproc.cvtColor(img, rgbFrame, Imgproc.COLOR_RGBA2RGB); // Cant't convert directly rgba->hsv
            //converting blured image from BGR to HSV
            Imgproc.cvtColor(rgbFrame, hsvImage, Imgproc.COLOR_RGB2HSV);

            //filtering red and green pixels based on given opencv HSV color range
            Core.inRange(hsvImage, new Scalar(24,50,50), new Scalar(34,255,255), mask1);
            Core.inRange(hsvImage, new Scalar(26,50,50), new Scalar(36,255,255), mask2);
...
//            Mat dst = new Mat();
            ...
            Mat res1=new Mat();
            Mat res2=new Mat();
            Core.bitwise_and(background,background,res1);
            Core.bitwise_and(img,img,res2);
            Mat final_output=new Mat();
            try {
                Core.addWeighted(res1,1,res2,1,0,final_output);
            }catch (Exception e){
